...
import gmhelper.test.GMBaseTest;
...
import org.hyperledger.gm.helper.BCECUtil;
import org.hyperledger.gm.helper.SM2Util;
import org.hyperledger.gm.helper.cert.*;
...
public class SM2CertUtilTest {
    ...
    public void testVerifyCSR() {
        try {
            ...
            PKCS10CertificationRequest rootCSR = CommonUtil.createCSR(rootDN, rootPub, rootKP.getPrivate(),
                    SM2X509CertMaker.SIGN_ALGO_SM3WITHSM2);

            String requestToPEM = certificationRequestToPEM(rootCSR);
            Assert.assertNotNull(requestToPEM);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
